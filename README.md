...
- Nine fox animation states with 44 playback frames assembled from 13 supplied sprites and derived transition poses, animated items/enemies, local generated tile art, parallax scenery, dust/sparkle/defeat effects, landing squash, hurt flashing, and camera shake.
...
| `tools/generate_player.py` / `assets/sprites/player_frames.tres` | Supplied-art importer and nine animation states |
...
The player now uses your 13 supplied fox sprites directly, preserving their face, swept tails, cream robe and burgundy trim. Feet are aligned across differently cropped images, with separate upright walking and leaning running cycles. Jump, fall, hurt and death remain derived poses rather than dedicated hand-drawn action sets. The controller adds quicker turns/stops, separate rise/fall gravity, dedicated interruptible landing animation, a cleaner dash exit, short buffered hops, and reliable high-speed stomps.
...
World graphics and audio were generated locally from original drawing/synthesis code. Player artwork uses the 13 user-provided PNGs, preserved in assets/sprites/source_fox with provenance hashes; additional transition poses are derived locally. The original large reference image and additional pose sheet are archived under `source_art/` for future animation work; they are excluded from the runtime resource pack. No downloaded asset packs or paid services were used.
